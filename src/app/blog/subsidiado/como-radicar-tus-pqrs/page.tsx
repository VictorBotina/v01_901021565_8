// src/app/blog/subsidiado/como-radicar-tus-pqrs/page.tsx
...
const ARTICLE_ID = "g9e7swnkq1unwxt1mzihy7tg";
const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:9002';
const shareUrl = `${siteUrl}/blog/subsidiado/como-radicar-tus-pqrs`;
...
async function getArticle(): Promise<Article | null> {
    try {
        const article = await fetchFromStrapi(`articles/${ARTICLE_ID}`, queryParams);
        return article;
    } catch (error) {
        console.error("❌ Falló la obtención del artículo:", error);
        return null;
    }
}
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    keywords: article.keywords, // Uso de keywords para posicionamiento
...
function renderArticleContent(content: Article['content']) {
  if (!content || !Array.isArray(content)) return <div className="text-center py-8 text-muted-foreground"><p>Contenido no disponible.</p></div>;
  return content.map((section, sectionIndex) => (
    <section key={section.id || sectionIndex} className="mb-10">
      {section.title_seccion && <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-6 pb-2 border-b border-gray-200">{section.title_seccion}</h2>}
            ...
            if (paragraphText.match(/^\d+\.\s/)) {
              return (
                <div key={textIndex} className="flex items-start space-x-3">
                  <span className="flex-shrink-0 w-6 h-6 bg-primary text-primary-foreground rounded-full text-sm flex items-center justify-center font-semibold mt-1">{paragraphText.split(".")[0]}</span>
            ...
            if (embedUrl) {
                return (
                    <div className="relative my-8" style={{ paddingBottom: '56.25%' }}>
                        <iframe src={embedUrl} title={section.title_seccion || "Video de YouTube"} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="absolute top-0 left-0 w-full h-full rounded-lg shadow-lg"></iframe>
                    </div>
                );
            }
            return null;
...
export default async function ComoRadicarPqrsPage() {
  const article = await getArticle();
  if (!article) notFound();
    ...
    <div className="bg-background">
      <article className="container mx-auto px-4 py-8 md:py-16 max-w-4xl">
        <header className="text-center mb-12">
           {article.category && <Link href={`/blog/subsidiado`} className="inline-flex items-center bg-primary/10 text-primary text-sm font-semibold px-4 py-2 rounded-full mb-6 hover:bg-primary/20 transition-colors">{article.category.name}</Link>}
          ...
          {article.description && <p className="text-xl text-muted-foreground mb-8 leading-relaxed max-w-3xl mx-auto">{article.description}</p>}
           <div className="flex flex-col md:flex-row items-center justify-center gap-x-6 gap-y-4 text-muted-foreground text-sm">
